...
app.UseSwagger();
app.UseSwaggerUI(c =>
    c.SwaggerEndpoint("/swagger/v1/swagger.json", $"Jira Management API v{appVersion}"));
...
app.UseExceptionHandler(exApp => exApp.Run(async context =>
{
    ...
    {
        logger.LogWarning(httpEx, "Jira API error on {Method} {Path}",
